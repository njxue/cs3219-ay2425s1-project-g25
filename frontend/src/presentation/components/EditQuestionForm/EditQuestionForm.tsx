...
import { categoryOptions, difficultyOptions } from "presentation/utils/QuestionUtils";
...
export const EditQuestionForm: React.FC<EditQuestionFormProps> = ({ question, onSubmit }) => {
    ...
    useEffect(() => {
        form.setFieldsValue(questionUpdateInput);
    }, [question]);
    ...
    async function handleSubmit(questionUpdate: IQuestionUpdateInput) {
        try {
            const res = await questionUseCases.updateQuestion(question?.questionId, questionUpdate);
